...
def average_weights(weight_dicts):
    ...
    for key in weight_dicts[0].keys():
        avg_weights[key] = sum([weights[key] for weights in weight_dicts]) / len(weight_dicts)
    return avg_weights



def optimize(agrs,mlp=None):
...
if __name__ == '__main__':
    ...
    num_seeds = 2  # Define the number of seeds
    all_weights = []
    ...
    mlp.load_state_dict(averaged_weights)
